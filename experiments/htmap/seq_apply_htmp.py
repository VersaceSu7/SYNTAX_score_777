...
from .seq_dataset_htmp import SyntaxFusionDataset
from .seq_model_htmp import SyntaxLightningModuleHeatmap
...
def run_variant_artery(
    ...
    with torch.no_grad():
        for x, h, [y], [t], [_weight_], [sid] in tqdm.tqdm(test_dataloader):
            if len(x.shape) == 1:
                bin_prob = 0.0
                val_syntax = 0.0
                bin_val = 0
